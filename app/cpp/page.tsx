...
import { languageOptions, lessonText, ui, type Language } from "../content";
import { academyUi, lessonGuides, taskInstructions } from "../curriculum";
import { buildFailureOutput } from "../diagnostics";
import { taskExplanations } from "../task-explanations";
import { taskCheckHints, taskGroups, taskOutputs } from "../tasks";
import { AccountButton } from "../account-button";
import { useCloudProgress } from "../cloud-progress";

const pipelineMeta = [
  { ext: ".cpp", color: "#ffcc66" },
  { ext: "#include", color: "#ef7c65" },
  { ext: ".obj", color: "#7ac7c4" },
  { ext: ".exe", color: "#8aa7ff" },
...
export default function Home() {
  ...
  const failureOutput = currentResult && !currentPassed
    ? buildFailureOutput({
        code: codes[taskIndex],
        checks: currentResult,
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      const savedLanguage = (window.localStorage.getItem("learning-park-language") || window.localStorage.getItem("cpp-park-language")) as Language | null;
      const savedCodes = window.localStorage.getItem("cpp-park-codes-60");
      const savedCompleted = window.localStorage.getItem("cpp-park-completed-60");
  ...
  useEffect(() => {
    if (loaded) window.localStorage.setItem("cpp-park-codes-60", JSON.stringify(codes));
  }, [codes, loaded]);

  useEffect(() => {
    if (loaded) window.localStorage.setItem("cpp-park-completed-60", JSON.stringify(completed));
  }, [completed, loaded]);
  ...
  return (
    <main className="cpp-course">
        ...
        <div className="learning-layout" id="practice">
          <article className="lesson-copy">
            ...
            <div className="mission-card">
              <p>{copy.mission}: {academy.task} {activeTask + 1} {academy.of} 10</p><strong>{taskInstructions[language][activeStation][activeTask]}</strong>
              <div className="concept-note"><span>{academy.conceptTitle}</span><p><InlineCode text={taskExplanations[language][activeStation][activeTask]} /></p></div>
          ...
          <div className="workbench">
            <div className="code-window interactive">
              <div className="code-topbar"><span><i></i><i></i><i></i></span><strong>station_{activeStation + 1}_task_{activeTask + 1}.cpp</strong><small>{copy.editor}</small></div>
              <div className="editor-shell"><div className="line-numbers" aria-hidden="true">{codes[taskIndex].split("\n").map((_, index) => <span key={index}>{index + 1}</span>)}</div><textarea value={codes[taskIndex]} onChange={(event) => updateCode(event.target.value)} spellCheck={false} aria-label={`${copy.editor} C++`} /></div>
            ...
            <div className={`program-output ${currentResult ? (currentPassed ? "success" : "error") : ""}`} aria-live="polite">
              ...
              <pre>{!currentResult ? academy.outputWaiting : currentPassed ? (taskOutputs[activeStation][activeTask] || academy.noConsoleOutput) : failureOutput}</pre>
      ...
      <footer><a className="brand footer-brand" href="#top"><span className="brand-mark">C<span>++</span></span><span className="brand-copy"><strong>{copy.brand}</strong><small>{copy.brandSub}</small></span></a><p>{copy.footer}</p><a href="#top">{copy.top} ↑</a></footer>
