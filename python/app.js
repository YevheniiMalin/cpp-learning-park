const { academyUi, buildPythonFailureOutput, buildPythonInstruction, languageOptions, pythonConceptExplanation, pythonLessonGuides: lessonGuides, pythonLessons: lessonText, pythonTaskGroups: taskGroups, pythonUi: ui } = PythonParkContent;
const allTasks = taskGroups.flat();
const pipelineMeta = [
  { ext: ".py", color: "#ffd75e" }, { ext: "name", color: "#55cbd3" },
  { ext: "syntax", color: "#ef7c65" }, { ext: ".pyc", color: "#8aa7ff" },
  { ext: ">>>", color: "#a9df8d" }
];
const stationTags = ["print", "=", "for", "def", "[]", "class"];
let language = localStorage.getItem("learning-park-language") || localStorage.getItem("python-park-language") || "en";
...
try {
  const savedCodes = JSON.parse(localStorage.getItem("python-park-codes-60") || "null");
  if (Array.isArray(savedCodes) && savedCodes.length === allTasks.length) codes = savedCodes;
  const savedCompleted = JSON.parse(localStorage.getItem("python-park-completed-60") || "null");
...
const saveProgress = () => {
  ...
  localStorage.setItem("python-park-codes-60", JSON.stringify(codes));
  localStorage.setItem("python-park-completed-60", JSON.stringify(completed));
};
...
function renderLesson() {
  ...
  byId("missionText").textContent = buildPythonInstruction(language, activeStation, currentTask(), activeTask);
  byId("conceptNoteLabel").textContent = academy.conceptTitle;
  byId("conceptNoteText").innerHTML = inlineCodeHtml(pythonConceptExplanation(language, activeStation));
  ...
  byId("codeFilename").textContent = `station_${activeStation + 1}_task_${activeTask + 1}.py`;
...
function renderTests() {
  ...
  byId("programOutputText").textContent = !current ? academy.outputWaiting : passed ? (currentTask().output || academy.noConsoleOutput) : buildPythonFailureOutput(codes[index], current, currentTask(), language);
...
function initCanvas() {
  ...
  const draw = () => {
    ...
    context.lineCap="round"; context.lineJoin="round"; context.beginPath(); for(let i=0;i<=90;i++){const point=track(i/90);if(i)context.lineTo(point.x,point.y);else context.moveTo(point.x,point.y)} context.strokeStyle="rgba(17,25,46,.72)";context.lineWidth=10;context.stroke();context.strokeStyle="#83dbd2";context.lineWidth=4;context.stroke();
